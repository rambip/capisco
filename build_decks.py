...
Local replacement for the (removed) build-decks CI workflow. For each requested
level it runs `pipeline.py --config configs/<lvl>.yml`, then uploads the resulting
`decks/*.json` to a GitHub release via the `gh` CLI (the viewer reads decks from the
latest release's .json assets).

Each level costs OpenRouter LLM calls, so build a level at a time when iterating.

Usage:
    uv run build_decks.py b1                 # build just B1
    uv run build_decks.py a1 a2 b1           # build several
    uv run build_decks.py all                # build every level
    uv run build_decks.py b1 --no-upload     # build locally, skip the release
"""
...
def main() -> None:
    ...
    for lv in levels:
        ...
        run(["uv", "run", "pipeline.py", "--config", str(cfg), "--input", str(args.input)])
